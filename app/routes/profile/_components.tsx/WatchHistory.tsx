...
export default function WatchHistory() {
  ...
  const query = useQuery<ListResult<WatchHistory>>({
    queryKey: ["watchHistory", user?.id, paginate.currentPage],
    queryFn: async () => {
      let resp = await pb
        .collection("history")
        .getList<WatchHistory>(paginate.currentPage, 10);
      return resp;
    },
    enabled: !!user,
  });
  ...
  return (
    <>
      ...
      <StatePaginator totalPages={query.data.totalPages} {...paginate} />
    </>
  );
}
